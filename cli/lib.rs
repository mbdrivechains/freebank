...
pub enum Command {
    // FreeBank (operator decision 2026-09-15): the chassis token/market CLI
    // commands (BitAssets, AMM, Dutch auctions) are removed; the underlying
    // variants are retained but rejected at validation.
...
#[command(name = "freebank-cli", author, version, about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
