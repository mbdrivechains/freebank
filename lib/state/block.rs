...
mod test {
    ...
    fn disconnect_bitasset_data_update() -> anyhow::Result<()> {
        ...
        // FreeBank (operator decision 2026-09-15): BitAsset transactions are
        // rejected by the consensus gate in `validate_filled_transaction`
        // (reached via `validate`/`prevalidate`). This test now exercises only
        // the DEAD `connect`/`disconnect_tip` RollBack round-trip for BitAsset
        // data (the state module is retained), so the gated `validate` pre-check
        // is dropped — it would return `Error::DisabledBitAsset`.
        {
            let mut rwtxn = env.write_txn()?;
