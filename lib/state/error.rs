...
pub enum Error {
    ...
    // FreeBank consensus gate: the chassis token/market features are disabled
    // on this chain (operator decision 2026-09-15). The types are retained for
    // serialization-tag stability and upstream-merge ability, but a tx that is
    // one of these families — or that spends/creates a coin of these kinds —
    // is rejected in both the mempool and block-connect validation paths.
    #[error("freebank: bitasset transactions are disabled on this chain")]
    DisabledBitAsset,
    #[error("freebank: amm transactions are disabled on this chain")]
    DisabledAmm,
