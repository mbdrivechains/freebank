...
pub const fn magic_bytes(network: Network) -> MagicBytes {
    // First 4 bytes are the FreeBank network magic (FB 42 94 C0 base).
    // Rightmost bits of the 4th byte is the network identifier.
    let b0 = 0b1111_1011;
    let b1 = 0b0100_0010;
    let b2 = 0b1001_0100;
    let mut b3 = 0b1100_0000;
    match network {
        Network::Regtest => (),
...
#[cfg(test)]
mod network_magic_tests {
    use super::*;
    use std::str::FromStr as _;

    // Network::Alphanet must parse from the lowercase "alphanet" token (the
    // BitWindow `--network alphanet` launch value; strum serialize_all lowercase).
    #[test]
    fn alphanet_parses_from_lowercase_token() {
        assert_eq!(Network::from_str("alphanet"), Ok(Network::Alphanet));
        // Sanity: the token is exactly what Display round-trips to.
        assert_eq!(Network::Alphanet.to_string(), "alphanet");
    }

    // Alphanet must carry its OWN distinct P2P magic, and every network's magic
    // must be pairwise distinct so peers on different networks cannot handshake.
    #[test]
    fn alphanet_magic_is_distinct() {
        let nets = [
            Network::Regtest,
            Network::Signet,
            Network::Forknet,
