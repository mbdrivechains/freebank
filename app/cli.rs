...
static DEFAULT_DATA_DIR: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| match dirs::data_dir() {
        ...
        Some(data_dir) => Some(data_dir.join("freebank")),
    });
...
#[command(name = "freebankd", author, version, about, long_about = None)]
pub(super) struct Cli {
    ...
    /// FreeBank's data directory, and `<VERSION>` is the FreeBank app version.
