...
#[rpc(client, server)]
pub trait Rpc {
    // FreeBank (operator decision 2026-09-15): the chassis token/market RPCs
    // (BitAssets, AMM, Dutch auctions) are removed. The corresponding
    // TransactionData/OutputContent variants are retained (serialization-tag
    // stability + upstream merge) but rejected at validation, so no RPC to
    // author or query them is exposed.
